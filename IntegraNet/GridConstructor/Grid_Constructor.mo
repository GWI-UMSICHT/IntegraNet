...
//_________________________________________________________________________________//
  ...
  replaceable model Demand_Consumer_1 =
      IntegraNet.Consumer.Consumer_combined.Data.Demand_Table_combined
                                               ...
                                               "Load-profile data table for buildings in upper row" annotation (choicesAllMatching=true,Dialog(tab="Consumer"),
      Placement(transformation(extent={{-8,12},{12,32}})));
  replaceable model Demand_Consumer_2 =
      IntegraNet.Consumer.Consumer_combined.Data.Demand_Table_combined
                                                                    constrainedby IntegraNet.Consumer.Consumer_combined.Base.Demand_combined
                                               "Load-profile data table for buildings in lower row" annotation (choicesAllMatching=true,Dialog(tab="Consumer", enable=second_row),
      Placement(transformation(extent={{-8,12},{12,32}})));
...
//  parameter Integer n=n_elements;

  parameter Integer n_elements=1 "|System layout|Number of basic grid elements" annotation(HideResult=true);

  parameter Boolean second_row=false "|System layout|Activate second row of consumers" annotation(choices(__Dymola_checkBox=true),Dialog(group="System layout"),HideResult=true);

public
  final parameter Integer n_consumer= n_elements + IntegraNet.Basics.Functions.calc_true_booleans(second_Consumer,n_elements) "Number of consumers inside a GridConstructor";


  // Technology parameters in Systems models
  parameter IntegraNet.GridConstructor.DataRecords.TechnologyMatrix Technologies_1[30]={IntegraNet.GridConstructor.DataRecords.TechnologyMatrix() for i in 1:30} "Technologies in upper row" annotation (Dialog(group="Systems"), HideResult=true);
  parameter IntegraNet.GridConstructor.DataRecords.TechnologyMatrix Technologies_2[30]={IntegraNet.GridConstructor.DataRecords.TechnologyMatrix() for i in 1:30} "Technologies in lower row" annotation (Dialog(group="Systems", enable=second_row), HideResult=true);

  parameter IntegraNet.GridConstructor.DataRecords.BoilerParameters BoilerParameters_1[30]={IntegraNet.GridConstructor.DataRecords.BoilerParameters() for i in 1:30} "Boiler parameters" annotation (Dialog(tab="Boiler"), HideResult=true);
  parameter IntegraNet.GridConstructor.DataRecords.BoilerParameters BoilerParameters_2[30]={IntegraNet.GridConstructor.DataRecords.BoilerParameters() for i in 1:30} "Boiler parameters" annotation (Dialog(tab="Boiler", enable=second_row), HideResult=true);

  parameter IntegraNet.GridConstructor.DataRecords.PVParameters PVParameters_1[30]={IntegraNet.GridConstructor.DataRecords.PVParameters() for i in 1:30} "PV parameters" annotation (Dialog(tab="PV"), HideResult=true);
  parameter IntegraNet.GridConstructor.DataRecords.PVParameters PVParameters_2[30]={IntegraNet.GridConstructor.DataRecords.PVParameters() for i in 1:30} "PV parameters" annotation (Dialog(tab="PV", enable=second_row), HideResult=true);

  parameter IntegraNet.GridConstructor.DataRecords.HeatPumpParameters HeatPumpParameters_1[30]={IntegraNet.GridConstructor.DataRecords.HeatPumpParameters() for i in 1:30} "Heat pump parameters in upper row" annotation (Dialog(tab="HeatPump"), HideResult=true);
  parameter IntegraNet.GridConstructor.DataRecords.HeatPumpParameters HeatPumpParameters_2[30]={IntegraNet.GridConstructor.DataRecords.HeatPumpParameters() for i in 1:30} "Heat pump parameters lower row" annotation (Dialog(tab="HeatPump", enable=second_row), HideResult=true);

  parameter IntegraNet.GridConstructor.DataRecords.ConsumerParameters Scaling_factor_consumer_1[30]={IntegraNet.GridConstructor.DataRecords.ConsumerParameters() for i in 1:30} "Consumer parameters and scaling factors in upper row" annotation (Dialog(tab="Consumer"), HideResult=true);
  parameter IntegraNet.GridConstructor.DataRecords.ConsumerParameters Scaling_factor_consumer_2[30]={IntegraNet.GridConstructor.DataRecords.ConsumerParameters() for i in 1:30} "Consumer parameters and scaling factors lower row" annotation (Dialog(tab="Consumer", enable=second_row), HideResult=true);
  ...
  parameter IntegraNet.GridConstructor.DataRecords.CHPParameters CHPParameters_1[30]={IntegraNet.GridConstructor.DataRecords.CHPParameters() for i in 1:30} "CHP parameters in upper row" annotation (Dialog(tab="CHP"), HideResult=true);
  parameter IntegraNet.GridConstructor.DataRecords.CHPParameters CHPParameters_2[30]={IntegraNet.GridConstructor.DataRecords.CHPParameters() for i in 1:30} "CHP parameters lower row" annotation (Dialog(tab="CHP", enable=second_row), HideResult=true);

  parameter IntegraNet.GridConstructor.DataRecords.SolarHeatingParameters SolarHeatingParameters_1[30]={IntegraNet.GridConstructor.DataRecords.SolarHeatingParameters() for i in 1:30} "Solar heating parameters in upper row" annotation (Dialog(tab="SolarThermal"), HideResult=true);
  parameter IntegraNet.GridConstructor.DataRecords.SolarHeatingParameters SolarHeatingParameters_2[30]={IntegraNet.GridConstructor.DataRecords.SolarHeatingParameters() for i in 1:30} "Solar heating parameters lower row" annotation (Dialog(tab="SolarThermal", enable=second_row), HideResult=true);

  parameter IntegraNet.GridConstructor.DataRecords.CablePipeParameters CablePipeParameters[30]={IntegraNet.GridConstructor.DataRecords.CablePipeParameters() for i in 1:30} "Parameters of cables and piping" annotation (Dialog(tab="Cables and Piping"), HideResult=true);
...
public
  ...
  SI.Temperature T_source_ambient=simCenter.ambientConditions.temperature.value + 273.15 "Temperature of ambient air as heat source"
                                                                                                                                    annotation(Dialog(tab="HeatPump",
                                                                                                                                                                group="Source temperature"),HideResult=true);
...
protected
    ...
    parameter SI.Impedance losses[:]=CablePipeParameters.losses "Length of the cable segments in the Basic_Grid_Elements" annotation(Dialog(tab="Cables and Piping"),HideResult=true);
   ...
   // parameter IntegraNet.Components.Electrical.Grid.Characteristics.LVCabletypes CableType[:]=CablePipeParameters.CableType "Type of low voltage cable in the Basic_Grid_Elements" annotation (Dialog(tab="Cables and Piping"),HideResult=true);
   ...
   //Boiler parameters
    ...
    parameter SI.Efficiency eta_oil_c1[:]= BoilerParameters_1.eta "Boiler's overall efficiency" annotation(Dialog(tab="Boiler"),HideResult=true);
    parameter SI.Efficiency eta_oil_c2[:]=BoilerParameters_2.eta "Boiler's overall efficiency" annotation(Dialog(tab="Boiler", enable=second_row),HideResult=true);
   ...
   // parameter TransiEnt.Producer.Electrical.Photovoltaics.Advanced_PV.Characteristics.Generic_Characteristics_PVModule PVModuleCharacteristics_c1[:]=PVParameters_1.PVModuleCharacteristics;
   // parameter TransiEnt.Producer.Electrical.Photovoltaics.Advanced_PV.Characteristics.Generic_Characteristics_PVModule PVModuleCharacteristics_c2[:]=PVParameters_2.PVModuleCharacteristics;
   ...
   parameter String PVModuleCharacteristics_c2[:]=PVParameters_2.PVModuleCharacteristics annotation(HideResult=true);
    ...
    parameter SI.Power Q_CHP_c2[:]=CHPParameters_2.Q_CHP "Heat output of CHP" annotation(Dialog(tab="CHP", enable=second_row,HideResult=true));
    ...
    parameter SI.Temp_C T_s_amb_CHP_c1[:]=CHPParameters_1.T_storage_amb "Assumed constant temperature in tank installation room" annotation(Dialog(tab="CHP"), HideResult=true);
    parameter SI.Temp_C T_s_amb_CHP_c2[:]=CHPParameters_2.T_storage_amb "Assumed constant temperature in tank installation room" annotation(Dialog(tab="CHP", enable=second_row),HideResult=true);
    ...
    parameter SI.Temp_C T_s_amb_HP_c1[:]=HeatPumpParameters_1.T_storage_amb "Assumed constant temperature in tank installation room in heat pump system" annotation(HideResult=true);
    parameter SI.Temp_C T_s_amb_HP_c2[:]=HeatPumpParameters_2.T_storage_amb "Assumed constant temperature in tank installation room in heat pump system" annotation(HideResult=true);
   ...
   //Solar heating parameters
    ...
    parameter IntegraNet.Basics.Types.FuelType fuel_boiler_ST_c1[:]=SolarHeatingParameters_1.fuel "Fuel used in boiler" annotation (Dialog(tab="SolarHeating"), HideResult=true);
    parameter IntegraNet.Basics.Types.FuelType fuel_boiler_ST_c2[:]=SolarHeatingParameters_2.fuel "Fuel used in boiler" annotation (Dialog(tab="SolarHeating",enable=second_row), HideResult=true);
  ...
  // to make model solveable, last element in grid needs to have gas pipe regardless of presence of gas technology
    ...
    parameter Integer gaspipe_add1[n_elements]=cat(1,gaspipe[1:n_elements-1], {1}) annotation(HideResult=true);

public
    parameter IntegraNet.GridConstructor.DataRecords.DHNParameters DHNParameters_Main[30]={IntegraNet.GridConstructor.DataRecords.DHNParameters() for i in 1:30} "Parameters of main pipes" annotation (Dialog(tab="DHN", HideResult=true));
    parameter IntegraNet.GridConstructor.DataRecords.DHNParameters DHNParameters_Consumer_1[30]={IntegraNet.GridConstructor.DataRecords.DHNParameters() for i in 1:30} "Parameters of house pipes for top Consumer" annotation (Dialog(tab="DHN", HideResult=true));
    parameter IntegraNet.GridConstructor.DataRecords.DHNParameters DHNParameters_Consumer_2[30]={IntegraNet.GridConstructor.DataRecords.DHNParameters() for i in 1:30} "Parameters of house pipes for bottom Consumer" annotation (Dialog(tab="DHN", HideResult=true));

    ///--- Initialization
protected
    parameter ClaRa.Basics.Units.Temperature dhn_T_start_supply = simCenter.T_supply "Temperature at start of the simulation. Only used to calculate h_nom and h_start"
                                                                                                                                                                       annotation(Dialog(tab="DHN", group="Initialisation"), HideResult=true);
    parameter ClaRa.Basics.Units.Temperature dhn_T_start_return = simCenter.T_return "Temperature at start of the simulation. Only used to calculate h_nom and h_start"
                                                                                                                                                                       annotation(Dialog(tab="DHN", group="Initialisation"), HideResult=true);
    parameter ClaRa.Basics.Units.Pressure dhn_p_start_supply = simCenter.p_nom[2] "Pressure in supply at start of the simulation"
                                                                                                                                 annotation(Dialog(tab="DHN", group="Initialisation"), HideResult=true);
    parameter ClaRa.Basics.Units.Pressure dhn_p_start_return = simCenter.p_nom[1] "Pressure in supply at start of the simulation"
                                                                                                                                 annotation(Dialog(tab="DHN", group="Initialisation"), HideResult=true);
public
...
protected
    parameter Integer activate_consumer_pipes = simCenter.activate_consumer_pipes "Activate / Deactivate house pipes for faster simulation. 1 = house pipes activated" annotation (Dialog(tab="DHN"),group="Initialisation",choices(__Dymola_checkBox=true), HideResult=true);

public
    replaceable model HeatTransfer =
      IntegraNet.Components.Heat.VolumesValvesFittings.Base.HT_Single_Buried_LX                                                   constrainedby IntegraNet.Components.Heat.VolumesValvesFittings.Base.HT_PlugFlow_Base_LX
    "HeatTransfer-Model choice for district heating" annotation (choicesAllMatching=true,Dialog(tab="DHN", group="HeatTransfer"), Placement(
...
protected
    ...
    parameter Integer dhnpipe_add1[:]=cat(1,dhnpipe[1:n_elements-1],{dhnpipe[n_elements]}) annotation(HideResult=true);
...
//     parameter SI.Length dhn_diameter_i_main[:] = DHNParameters_Main.diameter_i;
//     parameter SI.Length dhn_diameter_o_main[:]= DHNParameters_Main.diameter_o;
//     parameter SI.Length dhn_pipe_wall_thickness_main[:] = DHNParameters_Main.pipe_wall_thickness;
...
//     parameter SI.Length dhn_diameter_i_Consumer_1[:] = DHNParameters_Consumer_1.diameter_i;
//     parameter SI.Length dhn_diameter_o_Consumer_1[:] = DHNParameters_Consumer_1.diameter_o;
//     parameter SI.Length dhn_pipe_wall_thickness_Consumer_1[:] = DHNParameters_Consumer_1.pipe_wall_thickness;
    //Consumer_2
...
//     parameter SI.Length dhn_diameter_i_Consumer_2[:] = DHNParameters_Consumer_2.diameter_i;
//     parameter SI.Length dhn_diameter_o_Consumer_2[:] = DHNParameters_Consumer_2.diameter_o;
//     parameter SI.Length dhn_pipe_wall_thickness_Consumer_2[:] = DHNParameters_Consumer_2.pipe_wall_thickness;
...
protected
     parameter Real color_1 = if not second_row  then 255 else 28; // Sets color of circles used in Icon view to white
     parameter Real color_2 = if not second_row  then 255 else 108;// Sets color of circles used in Icon view to white
     parameter Real color_3 = if not second_row  then 255 else 200;// Sets color of circles used in Icon view to white
...
public
    replaceable model Systems_Consumer_1 =
      IntegraNet.GridConstructor.Systems.IndependentTechnologies
    constrainedby IntegraNet.GridConstructor.Systems.Base.Systems_Base                       "Systems model for buildings in upper row" annotation (choicesAllMatching=true, Dialog(group="Systems"),
      Placement(transformation(extent={{-8,12},{12,32}})));

   replaceable model Systems_Consumer_2 =
      IntegraNet.GridConstructor.Systems.IndependentTechnologies
    constrainedby IntegraNet.GridConstructor.Systems.Base.Systems_Base                        "Systems model for buildings in lower row (if onlyElectric selected in upper row, it needs to be onlyElectric in this row, too)"  annotation (choicesAllMatching=true, Dialog(group="Systems", enable=second_row),
      Placement(transformation(extent={{-8,12},{12,32}})));
 ...
 // 6. Parametrise the cable and gas pipe models with the respective parameter vectors
  ...
  IntegraNet.GridConstructor.Basic_Grid_Element Basic_Grid_Elements[n_elements](
    ...
    dhn_out_s = cat(1,fill(dhn_hidden_connectors,n_elements-1),{dhn_out_s}),
    dhn_in_r= cat(1,fill(dhn_hidden_connectors,n_elements-1),{dhn_in_r}),
    dhn_out_r = fill(dhn_out_r,n_elements),
    dhn_main_pipe = cat(1,dhnpipe[1:n_elements-1],{dhnpipe[n_elements]}),
    dhn_consumer_1 = cat(1,dhnpipe_c1[1:n_elements-1],{dhnpipe_c1[n_elements]}),
    dhn_consumer_2 = cat(1,dhnpipe_c2[1:n_elements-1],{dhnpipe_c2[n_elements]}),
    second_Consumer=second_Consumer,
    gas_pipe=gaspipe_add1,
    each activate_consumer_pipes = activate_consumer_pipes,
    cable(l=l,losses=losses),
    redeclare model Gas_Pipe =
        IntegraNet.Components.Gas.VolumesValvesFittings.PipeFlow_L4_Simple_varXi (
        each frictionAtOutlet=true,
        length=length,
    ...
    redeclare model Systems_Consumer_1 = Systems_Consumer_1 (
        ...
        eta_oil= eta_boiler_c1,
        eta_biomass= eta_boiler_c1,
    ...
    redeclare model Systems_Consumer_2 = Systems_Consumer_2 (
        ...
        eta_oil= eta_boiler_c2,
        eta_biomass= eta_boiler_c2,
        ...
        each T_source_constant=T_source_constant,
        each T_source_other=T_source_other),
        main_dhn_pipe(redeclare model HeatTransfer =
        HeatTransfer,
       DN = dhnpipe_DN_main,
       ...
       each T_start_return = dhn_T_start_return),
        house_pipe_Consumer_1(redeclare model HeatTransfer =
        HeatTransfer,
       ...
       each p_start_return = dhn_p_start_return,
       each T_start_return = dhn_T_start_return),
        house_pipe_Consumer_2(redeclare model HeatTransfer =
        HeatTransfer,
    ...
    redeclare model Demand_Consumer_1 = Demand_Consumer_1 (
        ...
        electricityDemand=factor_electricity_c1))
    annotation (Placement(transformation(extent={{-54,-14},
            {-22,16}})));

       // each T_return=dhn_T_start_supply,
        //each T_return=dhn_T_start_supply,
        //T_set
   // redeclare model Demand_Consumer_1 = Demand_Consumer_1 (consumer_count={i for i in start_c1:(n_elements + start_c1 - 1)}),
   // redeclare model Demand_Consumer_2 = Demand_Consumer_2 (consumer_count={i for i in start_c2:(n_elements + start_c2 - 1)}),
  //  redeclare model Demand_Consumer_2 = Demand_Consumer_2 (
  //      consumer_count={i for i in start_c2:(n_elements + start_c2 - 1)},
  //      heatDemand=factor_heat_c2,
  //      waterDemand=factor_warmwater_c2,
  ...
  // _____________________________________________
   ...
   SI.Velocity v_abs_mean "Average Abs Velocity of the water of the GridConstructor";
   Real dp_specific_mean "Average Specific pressure loss of the GridConstructor";
   SI.Temperature T_out_supply_mean "Average Temperatur of the Supply DHN Pipes";
   SI.Temperature T_out_return_mean "Average Temperatur of the Return DHN Pipes";
   Real Q_loss_specific_supply_mean "Average Specific Heatloss of the Supply Pipes [W/m]";
   Real Q_loss_specific_return_mean "Average Specific Heatloss of the Return Pipes [W/m]";
...
equation
  ...
  // _____________________________________________
   ...
   n_vrd  = sum(Basic_Grid_Elements[:].n_vrd);
   n_vrd_upper  = sum(Basic_Grid_Elements[:].n_vrd_upper);
   n_vrd_lower  = sum(Basic_Grid_Elements[:].n_vrd_lower);


   v_abs_mean = sum(Basic_Grid_Elements[:].get_Data[1].y)/n_elements;
   dp_specific_mean  = sum(Basic_Grid_Elements[:].get_Data[2].y)/n_elements;
   T_out_supply_mean = sum(Basic_Grid_Elements[:].get_Data[3].y)/n_elements;
   T_out_return_mean = sum(Basic_Grid_Elements[:].get_Data[4].y)/n_elements;
   Q_loss_specific_supply_mean = sum(Basic_Grid_Elements[:].get_Data[5].y)/n_elements;
   Q_loss_specific_return_mean = sum(Basic_Grid_Elements[:].get_Data[6].y)/n_elements;
  ...
  annotation (Diagram(graphics={
        ...
        Ellipse(extent={{-86,54},{-82,58}},
                                        lineColor={28,108,200}),
        Ellipse(extent={{-86,48},{-82,52}},
                                         lineColor={28,108,200}),
        Ellipse(extent={{-118,54},{-114,58}},
                                          lineColor={28,108,200}),
        Ellipse(extent={{-118,48},{-114,52}},
                                           lineColor={28,108,200}),
        Text(
          extent={{-58,-44},{-18,-28}},
          lineColor={28,108,200},
        ...
        Ellipse(extent={{-38,54},{-34,58}},
                                        lineColor={28,108,200}),
        Ellipse(extent={{-38,48},{-34,52}},
                                         lineColor={28,108,200}),
        Ellipse(extent={{-70,54},{-66,58}},
                                          lineColor={28,108,200}),
        ...
        Text(
          ...
          lineColor={28,108,200},
          textString="" + DynamicSelect("?", String(n_consumer))),     Line(points={{-106,40},{-106,0}}, color={28,108,200}),
        Ellipse(extent={{-84,-8},{-70,6}}, lineColor={28,108,200}),
...
")}),                                   Text(
      ...
      lineColor={0,0,0},
      textString="" + DynamicSelect("?", String(n_consumer))),                               defaultComponentName="grid",
    Documentation(info="<html>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">1. Purpose of model</span></b></p>
<p>Represents a Grid Segment (e. g. street) with multiple consumers and accociated technologies for heat and electricity production. </p>
