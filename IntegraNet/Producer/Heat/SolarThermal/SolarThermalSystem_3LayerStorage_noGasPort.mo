within IntegraNet.Producer.Heat.SolarThermal;
model SolarThermalSystem_3LayerStorage_noGasPort
//_________________________________________________________________________________//
// Component of the IntegraNet Library, version: 1.0.0                             //
//                                                                                 //
// Licensed by Fraunhofer UMSICHT and GWI Essen e.V. under Modelica License 2.     //
// Copyright 2021, Fraunhofer UMSICHT and GWI Essen e.V.                           //
//_________________________________________________________________________________//
//                                                                                 //
// IntegraNet is a research project supported by the German                        //
// Federal Ministry of Economics and Energy (FKZ 0324027).                         //
// The IntegraNet Library research team consists of the following project partners://
// Fraunhofer Institute for Environmental, Safety and Energy Technology UMSICHT,   //
// Gas- und Wärme-Institut Essen e.V.                                              //
// and is supported by                                                             //
// XRG Simulation GmbH (Hamburg, Germany)                                          //
//_________________________________________________________________________________//

  //electrical energy for the circulation pumps is not accounted for yet

  // _____________________________________________
  //
  //          Imports and Class Hierarchy
  // _____________________________________________
  outer IntegraNet.SimCenter simCenter;
  outer IntegraNet.Statistics.Statistics_collector statistics_collector;
  extends TransiEnt.Basics.Icons.Model;
  import FT=IntegraNet.Basics.Types.FuelType;

  // _____________________________________________
  //
  //           Parameters
  // _____________________________________________

   // parameter TILMedia.VLEFluidTypes.BaseVLEFluid FuelMedium=simCenter.gasModel1 "|Fundamental definitions|Fuel gas medium";
    parameter Boolean SpaceHeating=true "Does the solar heating system provide energy for space heating?" annotation(choices(checkBox=true));
    parameter SI.Temperature T_room=288 "Temperature of the installation room";
    parameter SI.Temperature T_return=308.15 "Return temperature of the heating system";
    parameter SI.Temperature T_boiler=60+273.15 "Temperature setpoint of the boiler";
    parameter SI.HeatFlowRate Q_flow_n_boiler=100000 "|Boiler|Nominal heating power of the gas boiler";
    parameter SI.Efficiency eta=1.05 "|Boiler|Boiler's overall efficiency";
    parameter SI.Irradiance G_min=150 "|Collector|minimum Irradiance before collector is working";
    parameter SI.Temperature T_set=348.15 "|Collector|Temperature set point for controller";
    parameter SI.Temperature T_max=273.15 + 95 "|Collector|maximum input temperature for collector switch-off";
    parameter SI.HeatFlowRate Q_flow_n=100e3 "|Collector|Nominal heat flow rate of the collector (for cost calculation)";
    parameter SI.Area area=5 "|Collector|Aperture area";
    parameter Real c_eff(unit="J/(m2.K)")=5000 "|Collector|Effective thermal capacity of the collector";
    parameter Real eta_0=0.793 "|Collector|Zero-loss collector efficiency";
    parameter Real a1(unit="W/(m2.K)")=4.04 "|Collector|Heat loss coefficient at (T_m - T_amb) = 0";
    parameter Real a2(unit="W/(m2.K2)")=0.0182 "|Collector|Temperature dependent heat loss coefficient";
    parameter SI.Angle longitude_standard=SI.Conversions.from_deg(15) "|Collector|needed for calculation of coordinated universal time (utc), 15 for central european time, 30 for central european summer time" annotation(HideResult=true);
    parameter SI.Conversions.NonSIunits.Time_day totaldays=365 "|Collector|total days of the year, standard=365, leap year=366" annotation(HideResult=true);
    parameter SI.Angle latitude=SI.Conversions.from_deg(53.55) "|Collector|latitude of the local position, north posiive, 53,55 North for Hamburg" annotation(HideResult=true);
    parameter SI.Angle slope=SI.Conversions.from_deg(53.55) "|Collector|slope of the tilted surface, assumption" annotation(HideResult=true);
    parameter SI.Angle surfaceAzimuthAngle=0 "|Collector|surface azimuth angle" annotation(HideResult=true);
replaceable model Skymodel =
      TransiEnt.Producer.Heat.SolarThermal.Base.Skymodel_HDKR constrainedby TransiEnt.Producer.Heat.SolarThermal.Base.SkymodelBase "|Collector|choose between HDKR and isotropic sky model" annotation (choicesAllMatching=true, Dialog(tab="Irradiance", group="Skymodel"));
    parameter Real reflectance_ground=0.2 "|Collector|reflectance of the ground" annotation(HideResult=true);
    parameter Boolean direct_normal=true "|Collector|Is the direct irradiance measured on a surface normal to irradiance?" annotation(HideResult=true);
    parameter SI.Angle longitude_local=SI.Conversions.from_deg(10) "|Collector|longitude of the local position, east positive, 10 East for Hamburg" annotation(HideResult=true);
    parameter SI.Volume Volume_tank= 2 "|Storage|Volume of the storage tank" annotation(HideResult=true);
    parameter Real p_Volume[3]={0.2,0.3,0.5} "|Storage|Proportion of the total volume for the three parts of the tank" annotation(HideResult=true);

    parameter SI.Temperature T_start[storage.N_cv]=fill(273.15 + 60, storage.N_cv) "|Storage|Temperatures at initalization" annotation(HideResult=true);

    parameter SI.Length h_tank=1 "|Storage|height of tank" annotation(HideResult=true);
    parameter SI.CoefficientOfHeatTransfer U_wall=0.5 "|Storage|Coefficient of heat transfer from wall to ambient" annotation(HideResult=true);
    parameter SI.ThermalConductivity k=0.6 "|Storage|Thermal conductivity of fluid in storage" annotation(HideResult=true);
    parameter SI.Density rho=1e3 "|Storage|Density of fluid in storage" annotation(HideResult=true);
    parameter SI.SpecificHeatCapacity c_v=4.185e3 "|Storage|Heat capacity of fluid in storage" annotation(HideResult=true);
//     replaceable model CostStatisticsModel =
//       TransiEnt.Components.Statistics.ConfigurationData.StorageCostSpecs.SmallHotWaterStorage
//                                                                                                "|Statistics| Model for global cost calculation of the storage"  annotation (choicesAllMatching=true, Dialog(group="Statistics"));
//     replaceable model CostRecordSolarThermal =
//       TransiEnt.Components.Statistics.ConfigurationData.GeneralCostSpecs.SolarThermal
//                                                                                        "|Statistics|Solar thermal cost specification" annotation (__Dymola_choicesAllMatching=true);
    parameter FT fuel=FT.Gas "choice of fuel" annotation(HideResult=true);
  // _____________________________________________
  //
  //           Variables
  // _____________________________________________

     SI.Energy E_heating;
     SI.Energy E_hotwater;
     SI.Energy E_solar;
     SI.Energy E_boiler;

     Real SolarFraction=E_solar/(E_hotwater+E_heating+0.001);

     SI.MassFlowRate m_flow_consumer;
     SI.MassFlowRate m_flow_hotwater;
     SI.Power Q_flow_boiler;

     SI.HeatFlowRate HeatingSolar;
     SI.MassFlowRate m_flow_fuel "Fuel mass flow rate";

     Real solarCoverage;

  // _____________________________________________
  //
  //           Interfaces
  // _____________________________________________

      TransiEnt.Basics.Interfaces.Thermal.HeatFlowRateIn Q_demand_heating annotation (Placement(transformation(
        extent={{-14,-14},{14,14}},
        rotation=270,
        origin={-20,100}), iconTransformation(
        extent={{-13,-13},{13,13}},
        rotation=270,
        origin={-39,95})));
      TransiEnt.Basics.Interfaces.Thermal.HeatFlowRateIn Q_demand_hotwater annotation (Placement(transformation(
        extent={{-14,-14},{14,14}},
        rotation=270,
        origin={20,100}), iconTransformation(
        extent={{-13,-13},{13,13}},
        rotation=270,
        origin={41,95})));
  // _____________________________________________
  //
  //           Instances of other Classes
  // _____________________________________________

  IntegraNet.Storage.Heat.HeatStorageStratified_constProp.StratifiedHotWaterStorage_L3_noFluidPorts storage(
    N_cv=3,
    Volume_segments=p_Volume*Volume_tank,
    V=Volume_tank,
    h=h_tank,
    T_start=T_start,
    U_wall=U_wall,
    k=k,
    rho=rho,
    c_v=c_v)
     annotation (Placement(transformation(extent={{-82,-14},{-60,8}})));
    //redeclare model CostStatisticsModel = CostStatisticsModel

  IntegraNet.Producer.Heat.SolarThermal.solarCollector_energybased solarCollector(
    G_min=controller.G_min,
    Q_flow_n=Q_flow_n,
    area=area,
    eta_0=eta_0,
    a1=a1,
    a2=a2,
    c_eff=c_eff,
    longitude_local=longitude_local,
    longitude_standard=longitude_standard,
    totaldays=totaldays,
    latitude=latitude,
    slope=slope,
    surfaceAzimuthAngle=surfaceAzimuthAngle,
    redeclare model Skymodel = Skymodel,
    reflectance_ground=reflectance_ground,
    direct_normal=direct_normal)
     annotation (Placement(transformation(extent={{62,-24},{80,-6}})));
    //redeclare model CostRecordSolarThermal = CostRecordSolarThermal

  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow2 annotation (Placement(transformation(extent={{-30,28},{-44,42}})));
  Modelica.Blocks.Sources.RealExpression heat_boiler(y=min(Q_flow_n_boiler, max(0, 5*4190*(T_boiler - storage.port[1].T))))   annotation (Placement(transformation(extent={{30,40},{14,52}})));
  Modelica.Blocks.Sources.RealExpression Q_heating1(y=-HeatingSolar - Q_heating2.y)        annotation (Placement(transformation(extent={{30,30},{14,42}})));

  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow annotation (Placement(transformation(extent={{-30,-2},{-44,12}})));
  Modelica.Blocks.Sources.RealExpression Q_solar1(y=-solarCollector.m_flow*4185*(solarCollector.T_out - storage.port[2].T)) annotation (Placement(transformation(extent={{30,8},{14,20}})));

  Modelica.Blocks.Sources.RealExpression Q_heating2(y=max(-HeatingSolar, -m_flow_consumer*4185*(storage.port[2].T - T_return)))                   annotation (Placement(transformation(extent={{30,-2},{14,10}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow1 annotation (Placement(transformation(extent={{-28,-30},{-42,-16}})));
  Modelica.Blocks.Sources.RealExpression Q_solar2(y=-solarCollector.m_flow*4185*(storage.port[2].T - storage.port[3].T)) annotation (Placement(transformation(extent={{30,-24},{14,-12}})));

  Modelica.Blocks.Sources.RealExpression Q_DHW3(y=max(-Q_demand_hotwater, min(0, -m_flow_hotwater*4185*(storage.port[3].T - 12 - 273.15)))) annotation (Placement(transformation(extent={{30,-34},{14,-22}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW2(y=max(-Q_demand_hotwater - Q_DHW3.y, min(0, -m_flow_hotwater*4185*(storage.port[2].T - storage.port[3].T)))) annotation (Placement(transformation(extent={{30,-12},{14,0}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW1(y=-(Q_demand_hotwater + Q_DHW2.y + Q_DHW3.y)) annotation (Placement(transformation(extent={{30,20},{14,32}})));

protected
   Modelica.Blocks.Math.Add3 add  annotation (Placement(transformation(extent={{-12,30},{-22,40}})));
   Modelica.Blocks.Math.Add3 add1    annotation (Placement(transformation(extent={{-12,0},{-22,10}})));
   Modelica.Blocks.Math.Add add2 annotation (Placement(transformation(extent={{-12,-28},{-22,-18}})));
   Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature(T=T_room) annotation (Placement(transformation(extent={{4,-4},{-4,4}},
       rotation=90,
        origin={-70,22})));
   Modelica.Blocks.Math.Gain P_drive2m_flow(k=-1)       annotation (Placement(transformation(extent={{66,52},{58,60}})));
   Modelica.Blocks.Sources.RealExpression T_in1(y=storage.port[3].T)  annotation (Placement(transformation(extent={{-6.5,-4.5},{6.5,4.5}},
        rotation=0,
        origin={54.5,16.5})));
   Modelica.Blocks.Sources.RealExpression heatFlowRate_boiler(y=Q_flow_boiler)  annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-52,-44})));

public
  GenericFuel2Heat.GenericBoiler boiler(eta=eta, FuelType=fuel) annotation (Placement(transformation(extent={{-18,-70},{2,-50}})));

  TransiEnt.Producer.Heat.SolarThermal.Control.ControllerPumpSolarCollectorTandG
                                                  controller(
    T_set=T_set,
    G_min=G_min,
    T_stor_max=T_max,
    P_drive_min(k=controller.m_flow_min))                    annotation (Placement(transformation(extent={{74,34},{100,54}})));

  Statistics.LocalCollector heat_output_solar_collector(typeOfResource=IntegraNet.Statistics.TypeOfResource.q_output_SolarThermal) annotation (Placement(transformation(extent={{-80,80},{-60,100}})));
  Statistics.LocalCollector heat_output_aux_collector(typeOfResource=IntegraNet.Statistics.TypeOfResource.q_output_aux_SolarThermal) annotation (Placement(transformation(extent={{-60,80},{-40,100}})));
equation

  // _____________________________________________
  //
  //            Characteristic equations
  // _____________________________________________

  Q_solar1.y+Q_solar2.y=der(E_solar);
  Q_flow_boiler=der(E_boiler);
  Q_demand_heating=der(E_heating);
  -Q_DHW1.y-Q_DHW2.y-Q_DHW3.y=der(E_hotwater);

  solarCoverage=if SpaceHeating then E_solar/(E_heating+E_hotwater+0.001) else E_solar/(E_hotwater+0.001);

   m_flow_hotwater=Q_demand_hotwater/(4185*(storage.port[1].T - (12 + 273.15)));
   m_flow_fuel=boiler.m_fuel;

 if SpaceHeating==true then
   HeatingSolar=Q_demand_heating;
   m_flow_consumer=Q_demand_heating/(4185*(storage.port[1].T - T_return));
   Q_flow_boiler=heat_boiler.y;
 else
   HeatingSolar=0;
   m_flow_consumer=0;
   Q_flow_boiler=heat_boiler.y + Q_demand_heating;
 end if;

  heat_output_solar_collector.flowCollector.unit_flow = Q_solar1.y+Q_solar2.y;
  connect(heat_output_solar_collector.flowCollector, statistics_collector.flowCollector[IntegraNet.Statistics.TypeOfResource.q_output_SolarThermal]);

  heat_output_aux_collector.flowCollector.unit_flow = heatFlowRate_boiler.y;
  connect(heat_output_aux_collector.flowCollector, statistics_collector.flowCollector[IntegraNet.Statistics.TypeOfResource.q_output_aux_SolarThermal]);

 // _____________________________________________
 //
 //            Connect statements
 // _____________________________________________

  connect(storage.heatPortAmbient, fixedTemperature.port) annotation (Line(points={{-71,6.35},{-70,6.35},{-70,18}},              color={191,0,0}));
  connect(prescribedHeatFlow2.Q_flow, add.y) annotation (Line(points={{-30,35},{-30,35},{-22.5,35}},            color={0,0,127}));
  connect(prescribedHeatFlow.Q_flow, add1.y) annotation (Line(points={{-30,5},{-22.5,5}},                         color={0,0,127}));
  connect(prescribedHeatFlow1.Q_flow, add2.y) annotation (Line(points={{-28,-23},{-22.5,-23}},                     color={0,0,127}));
  connect(heat_boiler.y, add.u1) annotation (Line(points={{13.2,46},{6,46},{6,39},{-11,39}}, color={0,0,127}));
  connect(Q_heating1.y, add.u2) annotation (Line(points={{13.2,36},{6,36},{6,35},{-11,35}},
                                                                                        color={0,0,127}));
  connect(Q_DHW1.y, add.u3) annotation (Line(points={{13.2,26},{6,26},{6,31},{-11,31}},       color={0,0,127}));
  connect(Q_solar1.y, add1.u1) annotation (Line(points={{13.2,14},{6,14},{6,9},{-11,9}},  color={0,0,127}));
  connect(Q_heating2.y, add1.u2) annotation (Line(points={{13.2,4},{-11,4},{-11,5}},             color={0,0,127}));
  connect(Q_DHW2.y, add1.u3) annotation (Line(points={{13.2,-6},{5.6,-6},{5.6,1},{-11,1}},            color={0,0,127}));
  connect(Q_solar2.y, add2.u1) annotation (Line(points={{13.2,-18},{2,-18},{2,-20},{-11,-20}},  color={0,0,127}));
  connect(Q_DHW3.y, add2.u2) annotation (Line(points={{13.2,-28},{14,-28},{4,-28},{2,-28},{2,-26},{-11,-26}},
                                                                                                  color={0,0,127}));
  connect(solarCollector.T_in, T_in1.y) annotation (Line(points={{64.16,-7.08},{64.16,6},{64,6},{64,16.5},{61.65,16.5}},
                                                                                                                     color={0,0,127}));

  connect(prescribedHeatFlow1.port, storage.port[3]) annotation (Line(points={{-42,-23},{-56,-23},{-56,4.66333},{-60.88,4.66333}},color={191,0,0}));
  connect(prescribedHeatFlow.port, storage.port[2]) annotation (Line(points={{-44,5},{-50,5},{-50,3.93},{-60.88,3.93}},color={191,0,0}));
  connect(prescribedHeatFlow2.port, storage.port[1]) annotation (Line(points={{-44,35},{-54,35},{-54,3.19667},{-60.88,3.19667}},color={191,0,0}));
  connect(T_in1.y, controller.T_in) annotation (Line(points={{61.65,16.5},{64,16.5},{64,46},{75.7333,46}},
                                                                                                      color={0,0,127}));
  connect(T_in1.y, controller.T_stor) annotation (Line(points={{61.65,16.5},{64,16.5},{64,34},{75.7333,34}},
                                                                                                        color={0,0,127}));
  connect(controller.G_total, solarCollector.G) annotation (Line(points={{75.7333,38.1667},{70,38.1667},{70,0},{78.92,0},{78.92,-7.26}}, color={0,0,127}));
  connect(solarCollector.T_out, controller.T_out) annotation (Line(points={{76.58,-7.26},{76.58,-8},{78,-8},{76,-8},{76,-2},{68,-2},{68,42.3333},{75.7333,42.3333}}, color={0,0,127}));
  connect(controller.P_drive, P_drive2m_flow.u) annotation (Line(points={{75.7333,49},{76,49},{76,56},{66.8,56}}, color={0,0,127}));
  connect(P_drive2m_flow.y, solarCollector.m_flow) annotation (Line(points={{57.6,56},{46,56},{46,0},{62.18,0},{62.18,-7.26}}, color={0,0,127}));

  connect(heatFlowRate_boiler.y, boiler.Q_Demand) annotation (Line(points={{-41,-44},{-8,-44},{-8,-50}},
                                                                                                color={0,0,127}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics={
        Line(
          points={{-20,36},{-34,44}},
          color={0,0,0},
          smooth=Smooth.None),
        Line(
          points={{16,50},{-40,-58}},
          smooth=Smooth.None,
          color={255,255,255}),
        Line(
          points={{34,50},{-22,-58}},
          smooth=Smooth.None,
          color={255,255,255}),
        Line(
          points={{50,50},{-6,-58}},
          smooth=Smooth.None,
          color={255,255,255}),
        Line(
          points={{66,50},{10,-58}},
          smooth=Smooth.None,
          color={255,255,255}),
        Polygon(
          points={{-26,24},{38,-6},{64,58},{14,88},{-26,24}},
          lineColor={0,0,0},
          smooth=Smooth.None,
          fillPattern=FillPattern.Solid,
          fillColor={95,95,95},
          lineThickness=0.5),
        Line(
          points={{-20,36},{38,4},{42,14},{-10,42},{-4,52},{46,24},{50,32},{0,60},{4,68},{52,40},{56,48},{8,76},{12,82},{92,34}},
          color={255,255,255},
          smooth=Smooth.None),
        Line(
          points={{68,54},{72,46},{64,46}},
          color={0,0,0},
          smooth=Smooth.None),
        Line(
          points={{-28,46},{-24,38},{-32,38}},
          color={0,0,0},
          smooth=Smooth.None),
        Line(
          points={{78,42},{62,52}},
          color={0,0,0},
          smooth=Smooth.None),
        Bitmap(
          extent={{-84,-48},{-36,26}},
          imageSource=""
               + "",
          fileName="modelica://TransiEnt/Images/thermalStorage.png"),
        Bitmap(
          extent={{-8,2},{62,-94}},
          imageSource="")}),
                                                                 Diagram(coordinateSystem(preserveAspectRatio=false)),
    Documentation(info="<html>
<h4><span style=\"color: #008000\">1. Purpose of model</span></h4>
<p>Combination of storage tank, solar collector, controller and gas boiler</p>
<h4><span style=\"color: #008000\">2. Level of detail, physical effects considered, and physical insight</span></h4>
<p>(Purely technical component without physical modeling.)</p>
<h4><span style=\"color: #008000\">3. Limits of validity </span></h4>
<p>(Purely technical component without physical modeling.)</p>
<h4><span style=\"color: #008000\">4. Interfaces</span></h4>
<p>(no remarks)</p>
<h4><span style=\"color: #008000\">5. Nomenclature</span></h4>
<p>(no elements)</p>
<h4><span style=\"color: #008000\">6. Governing Equations</span></h4>
<p>(no equations)</p>
<h4><span style=\"color: #008000\">7. Remarks for Usage</span></h4>
<p>All components use energy based modeling without consideration of fluid flow. </p>
<p>Heat flow into the storage tank is based on the difference between the temperature of each tank level and the respective source or sink temperature. </p>
<h4><span style=\"color: #008000\">8. Validation</span></h4>
<p>(no validation or testing necessary)</p>
<h4><span style=\"color: #008000\">9. References</span></h4>
<p>(no remarks)</p>
<h4><span style=\"color: #008000\">10. Version History</span></h4>
<p>Model created by Anne Hagemeier (anne.hagemeier@umsicht.fraunhofer.de), Nov 2019</p>
</html>"));
end SolarThermalSystem_3LayerStorage_noGasPort;
