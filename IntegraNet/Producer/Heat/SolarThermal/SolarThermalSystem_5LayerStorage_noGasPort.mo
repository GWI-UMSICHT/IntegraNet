...
model SolarThermalSystem_5LayerStorage_noGasPort
...
//_________________________________________________________________________________//
  ...
  // _____________________________________________
    ...
    parameter SI.HeatFlowRate Q_flow_n_boiler=100000 "|Boiler|Nominal heating power of the gas boiler";
    ...
    parameter SI.Angle longitude_standard=SI.Conversions.from_deg(15) "|Collector|needed for calculation of coordinated universal time (utc), 15 for central european time, 30 for central european summer time" annotation(HideResult=true);
    parameter SI.Conversions.NonSIunits.Time_day totaldays=365 "|Collector|total days of the year, standard=365, leap year=366" annotation(HideResult=true);
    parameter SI.Angle latitude=SI.Conversions.from_deg(53.55) "|Collector|latitude of the local position, north posiive, 53,55 North for Hamburg" annotation(HideResult=true);
    parameter SI.Angle slope=SI.Conversions.from_deg(53.55) "|Collector|slope of the tilted surface, assumption" annotation(HideResult=true);
    parameter SI.Angle surfaceAzimuthAngle=0 "|Collector|surface azimuth angle" annotation(HideResult=true);
    ...
    parameter Real reflectance_ground=0.2 "|Collector|reflectance of the ground" annotation(HideResult=true);
    parameter Boolean direct_normal=true "|Collector|Is the direct irradiance measured on a surface normal to irradiance?" annotation(HideResult=true);
    parameter SI.Angle longitude_local=SI.Conversions.from_deg(10) "|Collector|longitude of the local position, east positive, 10 East for Hamburg" annotation(HideResult=true);
    parameter SI.Volume Volume_tank= 2 "|Storage|Volume of the storage tank" annotation(HideResult=true);
    parameter Real p_Volume[5]={0.3,0.3,0.1,0.1,0.2} "|Storage|Proportion of the total volume for the three parts of the tank" annotation(HideResult=true);

    parameter SI.Temperature T_start[storage.N_cv]=fill(273.15 + 60, storage.N_cv) "|Storage|Temperatures at initalization" annotation(HideResult=true);

    parameter SI.Length h_tank=1 "|Storage|height of tank" annotation(HideResult=true);
    parameter SI.CoefficientOfHeatTransfer U_wall=0.5 "|Storage|Coefficient of heat transfer from wall to ambient" annotation(HideResult=true);
    parameter SI.ThermalConductivity k=0.6 "|Storage|Thermal conductivity of fluid in storage" annotation(HideResult=true);
    parameter SI.Density rho=1e3 "|Storage|Density of fluid in storage" annotation(HideResult=true);
    parameter SI.SpecificHeatCapacity c_v=4.185e3 "|Storage|Heat capacity of fluid in storage" annotation(HideResult=true);
...
//                                                                                        "|Statistics|Solar thermal cost specification" annotation (__Dymola_choicesAllMatching=true);
  ...
  // _____________________________________________
     ...
     SI.MassFlowRate m_flow_heating;
  ...
  IntegraNet.Storage.Heat.HeatStorageStratified_constProp.StratifiedHotWaterStorage_L3_noFluidPorts storage(
    ...
    rho=rho,
    c_v=c_v)
     annotation (Placement(transformation(extent={{-84,-14},{-62,8}})));
  ...
  IntegraNet.Producer.Heat.SolarThermal.solarCollector_energybased solarCollector(
    ...
    direct_normal=direct_normal)
     annotation (Placement(transformation(extent={{62,-24},{80,-6}})));
...
protected
   Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature(T=T_room) annotation (Placement(transformation(extent={{4,-4},{-4,4}},
       rotation=90,
        origin={-70,22})));
   Modelica.Blocks.Math.Gain P_drive2m_flow(k=-1)       annotation (Placement(transformation(extent={{66,52},{58,60}})));
   Modelica.Blocks.Sources.RealExpression T_in(y=storage.port[5].T) annotation (
   ...
   Modelica.Blocks.Sources.RealExpression T_stor(y=storage.port[1].T)
    annotation (Placement(transformation(
        extent={{-6.5,-4.5},{6.5,4.5}},
        rotation=0,
        origin={34.5,38.5})));
public
  GenericFuel2Heat.GenericBoiler boiler(eta=eta, FuelType=fuel) annotation (Placement(transformation(extent={{-18,-92},{2,-72}})));
  ...
  Statistics.LocalCollector heat_output_solar_collector(typeOfResource=IntegraNet.Statistics.TypeOfResource.q_output_SolarThermal) annotation (Placement(transformation(extent={{-80,80},{-60,100}})));
  Statistics.LocalCollector heat_output_aux_collector(typeOfResource=IntegraNet.Statistics.TypeOfResource.q_output_aux_SolarThermal) annotation (Placement(transformation(extent={{-60,80},{-40,100}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow1 annotation (Placement(transformation(extent={{-28,54},{-42,68}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow2 annotation (Placement(transformation(extent={{-28,22},{-42,36}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow3
    annotation (Placement(transformation(extent={{-26,-2},{-40,12}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow4 annotation (Placement(transformation(extent={{-26,-28},
            {-40,-14}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow5 annotation (Placement(transformation(extent={{-26,-54},
            {-40,-40}})));
  Modelica.Blocks.Math.Add3 add1 annotation (Placement(transformation(extent={{-10,56},{-20,66}})));
  Modelica.Blocks.Math.Add3 add2 annotation (Placement(transformation(extent={{-10,24},{-20,34}})));
  Modelica.Blocks.Math.Add  add3
                                annotation (Placement(transformation(extent={{-10,-2},
            {-20,8}})));
  Modelica.Blocks.Math.Add add4 annotation (Placement(transformation(extent={{-10,-26},{-20,-16}})));
  Modelica.Blocks.Math.Add  add5
                                annotation (Placement(transformation(extent={{-10,-52},
            {-20,-42}})));
  Modelica.Blocks.Sources.RealExpression Q_boiler(y=min(Q_flow_n_boiler, max(0, 5*4190*(T_boiler - storage.port[1].T)))) annotation (Placement(transformation(extent={{18,66},
            {2,78}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW1(y=-(Q_demand_hotwater + Q_DHW2.y
         + Q_DHW3.y + Q_DHW4.y + Q_DHW5.y))
    annotation (Placement(transformation(extent={{18,56},{2,68}})));
  Modelica.Blocks.Sources.RealExpression Q_heating1(y=-HeatingSolar - Q_heating2.y)        annotation (Placement(transformation(extent={{18,46},
            {2,58}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW2(y=max(-Q_demand_hotwater -
        Q_DHW5.y - Q_DHW4.y - Q_DHW3.y, min(0, -m_flow_hotwater*4185*(storage.port[
        2].T - storage.port[3].T))))
    annotation (Placement(transformation(extent={{18,34},{2,46}})));
  Modelica.Blocks.Sources.RealExpression Q_heating2(y=max(-HeatingSolar, -m_flow_heating*4185*(storage.port[2].T - T_return)))                   annotation (Placement(transformation(extent={{18,24},
            {2,36}})));
  Modelica.Blocks.Sources.RealExpression Q_solar2(y=max(0, min(-solarCollector.Q_flow_out, -solarCollector.m_flow*4185*(solarCollector.T_out - storage.port[2].T))))
                                                                                                                     annotation (Placement(transformation(extent={{18,14},
            {2,26}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW3(y=max(-Q_demand_hotwater - Q_DHW5.y - Q_DHW4.y, min(0, -m_flow_hotwater*4185*(storage.port[3].T - storage.port[4].T))))
                 annotation (Placement(transformation(extent={{18,2},{2,14}})));
  Modelica.Blocks.Sources.RealExpression Q_solar3(y=min(-solarCollector.Q_flow_out, -solarCollector.m_flow*4185*(storage.port[2].T - storage.port[3].T)))
                                                                                                                     annotation (Placement(transformation(extent={{18,-8},
            {2,4}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW4(y=max(-Q_demand_hotwater - Q_DHW5.y, min(0, -m_flow_hotwater*4185*(storage.port[4].T - storage.port[5].T))))
    annotation (Placement(transformation(extent={{18,-22},{2,-10}})));
  Modelica.Blocks.Sources.RealExpression Q_solar4(y=max(0, min(-solarCollector.Q_flow_out - Q_solar2.y - Q_solar3.y, -solarCollector.m_flow*4185*(storage.port[3].T - storage.port[4].T))))
                                                                                                                     annotation (Placement(transformation(extent={{18,-32},{2,-20}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW5(y=max(-Q_demand_hotwater, min(0, -m_flow_hotwater*4185*(storage.port[5].T - 12 - 273.15))))
    annotation (Placement(transformation(extent={{18,-48},{2,-36}})));
  Modelica.Blocks.Sources.RealExpression Q_solar5(y=max(0, min(-solarCollector.Q_flow_out - Q_solar2.y - Q_solar3.y - Q_solar4.y, -solarCollector.m_flow*4185*(storage.port[4].T - storage.port[5].T))))
...
equation
  ...
  Q_solar2.y+Q_solar3.y+Q_solar4.y+Q_solar5.y=der(E_solar);
  Q_flow_boiler=der(E_boiler);
  Q_demand_heating=der(E_heating);
  -Q_DHW1.y-Q_DHW2.y-Q_DHW3.y-Q_DHW4.y-Q_DHW5.y=der(E_hotwater);

  solarCoverage=if SpaceHeating then E_solar/(E_heating+E_hotwater+0.001) else E_solar/(E_hotwater+0.001);

   m_flow_hotwater=Q_demand_hotwater/(4185*(T_boiler - (12 + 273.15)));
   m_flow_fuel=boiler.m_fuel;

 if SpaceHeating==true then
   HeatingSolar=Q_demand_heating;
   m_flow_heating=Q_demand_heating/(4185*(T_boiler - T_return));
   Q_flow_boiler=Q_boiler.y;
 else
   HeatingSolar=0;
   m_flow_heating=0;
   Q_flow_boiler=Q_boiler.y + Q_demand_heating;
 end if;

  heat_output_solar_collector.flowCollector.unit_flow = Q_solar2.y+Q_solar3.y+Q_solar4.y+Q_solar5.y;
  connect(heat_output_solar_collector.flowCollector, statistics_collector.flowCollector[IntegraNet.Statistics.TypeOfResource.q_output_SolarThermal]);

  heat_output_aux_collector.flowCollector.unit_flow = heatFlowRate_boiler.y;
  connect(heat_output_aux_collector.flowCollector, statistics_collector.flowCollector[IntegraNet.Statistics.TypeOfResource.q_output_aux_SolarThermal]);
 ...
 // _____________________________________________
  ...
  connect(storage.heatPortAmbient, fixedTemperature.port) annotation (Line(points={{-73,
          6.35},{-70,6.35},{-70,18}},                                                                                            color={191,0,0}));

  connect(controller.G_total, solarCollector.G) annotation (Line(points={{75.7333,38.1667},{70,38.1667},{70,0},{78.92,0},{78.92,-7.26}}, color={0,0,127}));
  connect(solarCollector.T_out, controller.T_out) annotation (Line(points={{76.58,-7.26},{76.58,-8},{78,-8},{76,-8},{76,-2},{68,-2},{68,42.3333},{75.7333,42.3333}}, color={0,0,127}));
  connect(controller.P_drive, P_drive2m_flow.u) annotation (Line(points={{75.7333,49},{76,49},{76,56},{66.8,56}}, color={0,0,127}));
  connect(P_drive2m_flow.y, solarCollector.m_flow) annotation (Line(points={{57.6,56},
          {52,56},{52,2},{62.18,2},{62.18,-7.26}},                                                                             color={0,0,127}));

  connect(heatFlowRate_boiler.y, boiler.Q_Demand) annotation (Line(points={{-37,-72},{-8,-72}}, color={0,0,127}));
  connect(storage.port[1], prescribedHeatFlow1.port) annotation (Line(points={{-62.88,3.05},{-62.88,34},{-60,34},{-60,60},{-42,60},{-42,61}}, color={191,0,0}));
  connect(storage.port[2], prescribedHeatFlow2.port) annotation (Line(points={{-62.88,3.49},{-62.88,16},{-56,16},{-56,28},{-42,28},{-42,29}},
                                                                                                                                    color={191,0,0}));
  connect(storage.port[3], prescribedHeatFlow3.port) annotation (Line(points={{-62.88,
          3.93},{-49.44,3.93},{-49.44,5},{-40,5}},                                                                             color={191,0,0}));
  connect(prescribedHeatFlow4.port, storage.port[4]) annotation (Line(points={{-40,-21},
          {-52,-21},{-52,4.37},{-62.88,4.37}},                                                                               color={191,0,0}));
  connect(prescribedHeatFlow5.port, storage.port[5]) annotation (Line(points={{-40,-47},
          {-58,-47},{-58,4.81},{-62.88,4.81}},                                                                               color={191,0,0}));
  connect(prescribedHeatFlow1.Q_flow, add1.y) annotation (Line(points={{-28,61},{-20.5,61}}, color={0,0,127}));
  connect(prescribedHeatFlow2.Q_flow, add2.y) annotation (Line(points={{-28,29},{-20.5,29}}, color={0,0,127}));
  connect(prescribedHeatFlow3.Q_flow,add3. y) annotation (Line(points={{-26,5},{
          -24,5},{-24,3},{-20.5,3}},                                                                       color={0,0,127}));
  connect(add4.y, prescribedHeatFlow4.Q_flow) annotation (Line(points={{-20.5,-21},
          {-26,-21}},                                                                          color={0,0,127}));
  connect(prescribedHeatFlow5.Q_flow,add5. y) annotation (Line(points={{-26,-47},
          {-20.5,-47}},                                                                        color={0,0,127}));
  connect(Q_boiler.y, add1.u1) annotation (Line(points={{1.2,72},{0,72},{0,65},{-9,65}}, color={0,0,127}));
  connect(Q_DHW1.y, add1.u2) annotation (Line(points={{1.2,62},{-4,62},{-4,61},{-9,61}}, color={0,0,127}));
  connect(Q_heating1.y, add1.u3) annotation (Line(points={{1.2,52},{-4,52},{-4,57},{-9,57}}, color={0,0,127}));
  connect(Q_DHW2.y, add2.u1) annotation (Line(points={{1.2,40},{-4,40},{-4,33},{-9,33}}, color={0,0,127}));
  connect(Q_heating2.y, add2.u2) annotation (Line(points={{1.2,30},{-4,30},{-4,29},{-9,29}}, color={0,0,127}));
  connect(Q_solar2.y, add2.u3) annotation (Line(points={{1.2,20},{-4,20},{-4,24},{-9,24},{-9,25}}, color={0,0,127}));
  connect(Q_DHW3.y,add3. u1) annotation (Line(points={{1.2,8},{-4,8},{-4,6},{-9,
          6}},                                                                         color={0,0,127}));
  connect(add3.u2, Q_solar3.y) annotation (Line(points={{-9,0},{-4,0},{-4,-2},{1.2,
          -2}},                                                          color={0,0,127}));
  connect(Q_DHW4.y,add4. u1) annotation (Line(points={{1.2,-16},{-4,-16},{-4,-18},
          {-9,-18}},                                                                         color={0,0,127}));
  connect(Q_solar4.y,add4. u2) annotation (Line(points={{1.2,-26},{-4,-26},{-4,-24},{-9,-24}}, color={0,0,127}));
  connect(Q_DHW5.y,add5. u1) annotation (Line(points={{1.2,-42},{-4,-42},{-4,-44},
          {-9,-44}},                                                                         color={0,0,127}));
  connect(Q_solar5.y,add5. u2) annotation (Line(points={{1.2,-52},{-2,-52},{-2,-50},
          {-9,-50}}, color={0,0,127}));
  connect(controller.T_in, T_in.y) annotation (Line(points={{75.7333,46},{56,46},{56,30.5},{41.65,30.5}},
                                   color={0,0,127}));
  connect(T_in.y, solarCollector.T_in) annotation (Line(points={{41.65,30.5},{56,
          30.5},{56,6},{64.16,6},{64.16,-7.08}}, color={0,0,127}));
  connect(T_stor.y, controller.T_stor) annotation (Line(points={{41.65,38.5},{60,38.5},{60,34},{75.7333,34}},
                                       color={0,0,127}));
...
end SolarThermalSystem_5LayerStorage_noGasPort;
