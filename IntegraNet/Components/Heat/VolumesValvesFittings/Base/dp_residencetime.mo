...
model dp_residencetime "Calculates the residence time aswell as the pressure loss and delays enthalpy changes according to it"
...
equation
  ...
  der(x) = v_water;
  (time_reversed,time_flow) = spatialDistribution(time,time,x/length,noEvent(v_water>=0), {1/v_nom,1.0},{ if calc_initial_dstrb then -((Modelica.Constants.pi*(1/4))*diameter_i^2*rho*cp_w) else 0, if calc_initial_dstrb then -((Modelica.Constants.pi*(1/4))*diameter_i^2*rho*cp_w) else 0});// x normalised
