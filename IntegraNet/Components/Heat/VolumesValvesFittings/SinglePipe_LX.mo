...
model SinglePipe_LX "Model of a pipe for use in district heating networks"
...
//_________________________________________________________________________________//
 ...
 // _____________________________________________
  replaceable model HeatTransfer =
      Base.HT_Single_Buried_LX                                 constrainedby Base.HT_PlugFlow_Base_LX           annotation (choicesAllMatching=true,Dialog(group="HeatTransfer"));
  inner Base.pipe_parameters pipe_parameter(
    calc_initial_dstrb=calc_initial_dstrb,  diameter_i = diameter_i,
    p_start=p_start,
    T_start=T_start,
  ...
  TransiEnt.Basics.Interfaces.Thermal.FluidPortOut waterPortOut(each Medium=
        simCenter.fluid1) "Outlet port" annotation (Placement(transformation(
          extent={{90,-10},{110,10}}), iconTransformation(extent={{90,-10},{110,
            10}})));
  Base.dp_residencetime dp_residencetime(calc_initial_dstrb=calc_initial_dstrb) annotation (Placement(transformation(extent={{-78,-16},{-42,16}})));
