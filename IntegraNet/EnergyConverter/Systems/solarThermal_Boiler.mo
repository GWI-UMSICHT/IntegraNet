within IntegraNet.EnergyConverter.Systems;
model solarThermal_Boiler "Solar heating and gas boiler"
//_________________________________________________________________________________//
// Component of the IntegraNet Library, version: 1.0.0                             //
//                                                                                 //
// Licensed by Fraunhofer UMSICHT and GWI Essen e.V. under Modelica License 2.     //
// Copyright 2021, Fraunhofer UMSICHT and GWI Essen e.V.                           //
//_________________________________________________________________________________//
//                                                                                 //
// IntegraNet is a research project supported by the German                        //
// Federal Ministry of Economics and Energy (FKZ 0324027).                         //
// The IntegraNet Library research team consists of the following project partners://
// Fraunhofer Institute for Environmental, Safety and Energy Technology UMSICHT,   //
// Gas- und Wärme-Institut Essen e.V.                                              //
// and is supported by                                                             //
// XRG Simulation GmbH (Hamburg, Germany)                                          //
//_________________________________________________________________________________//


  //electrical energy for the circulation pumps is not accounted for yet

  // _____________________________________________
  //
  //          Imports and Class Hierarchy
  // _____________________________________________

  extends IntegraNet.EnergyConverter.Systems.Base.Systems(
    final DHN=false,
    final gas_grid=true,
    final el_grid=true,
    medium1=FuelMedium);

  // _____________________________________________
  //
  //           Parameters
  // _____________________________________________

    parameter TILMedia.VLEFluidTypes.BaseVLEFluid FuelMedium=simCenter.gasModel1 "|Fundamental definitions|Fuel gas medium";
    parameter Boolean SpaceHeating=true "Does the solar heating system provide energy for space heating?" annotation(HideResult=true, choices(checkBox=true));
    parameter SI.Temperature T_room=288 "Temperature of the installation room" annotation(HideResult=true);
    parameter SI.Temperature T_return=308.15 "Return temperature of the heating system" annotation(HideResult=true);
    parameter SI.Temperature T_boiler=60+273.15 "Temperature setpoint of the boiler" annotation(HideResult=true);
    parameter SI.HeatFlowRate Q_flow_n_boiler=20000 "|Boiler|Nominal heating power of the gas boiler" annotation(HideResult=true);
    parameter SI.Efficiency eta=1.05 "|Boiler|Boiler's overall efficiency" annotation(HideResult=true);
    parameter Boolean referenceNCV=true "true, if heat calculations shall be in respect to NCV, false will give GCV" annotation(HideResult=true);
    parameter SI.Irradiance G_min=150 "|Collector|minimum Irradiance before collector is working" annotation(HideResult=true);
    parameter SI.Temperature T_set=348.15 "|Collector|Temperature set point for controller" annotation(HideResult=true);
    parameter SI.Temperature T_max=273.15 + 95 "|Collector|maximum input temperature for collector switch-off" annotation(HideResult=true);
    parameter SI.HeatFlowRate Q_flow_n=100e3 "|Collector|Nominal heat flow rate of the collector (for cost calculation)" annotation(HideResult=true);
    parameter SI.Area area=5 "|Collector|Aperture area" annotation(HideResult=true);
    parameter Real c_eff(unit="J/(m2.K)")=5000 "|Collector|Effective thermal capacity of the collector" annotation(HideResult=true);
    parameter Real eta_0=0.793 "|Collector|Zero-loss collector efficiency" annotation(HideResult=true);
    parameter Real a1(unit="W/(m2.K)")=4.04 "|Collector|Heat loss coefficient at (T_m - T_amb) = 0" annotation(HideResult=true);
    parameter Real a2(unit="W/(m2.K)")=0.0182 "|Collector|Temperature dependent heat loss coefficient" annotation(HideResult=true);
    parameter SI.Angle longitude_standard=SI.Conversions.from_deg(15) "|Collector|needed for calculation of coordinated universal time (utc), 15 for central european time, 30 for central european summer time" annotation(HideResult=true);
    parameter SI.Conversions.NonSIunits.Time_day totaldays=365 "|Collector|total days of the year, standard=365, leap year=366" annotation(HideResult=true);
    parameter SI.Angle latitude=SI.Conversions.from_deg(53.55) "|Collector|latitude of the local position, north posiive, 53,55 North for Hamburg" annotation(HideResult=true);
    parameter SI.Angle slope=SI.Conversions.from_deg(53.55) "|Collector|slope of the tilted surface, assumption" annotation(HideResult=true);
    parameter SI.Angle surfaceAzimuthAngle=0 "|Collector|surface azimuth angle" annotation(HideResult=true);
    replaceable model Skymodel =
      TransiEnt.Producer.Heat.SolarThermal.Base.Skymodel_HDKR constrainedby TransiEnt.Producer.Heat.SolarThermal.Base.SkymodelBase "|Collector|choose between HDKR and isotropic sky model" annotation (choicesAllMatching=true, Dialog(tab="Irradiance", group="Skymodel"));
    parameter Real reflectance_ground=0.2 "|Collector|reflectance of the ground" annotation(HideResult=true);
    parameter Boolean direct_normal=true "|Collector|Is the direct irradiance measured on a surface normal to irradiance?" annotation(HideResult=true);
    parameter SI.Angle longitude_local=SI.Conversions.from_deg(10) "|Collector|longitude of the local position, east positive, 10 East for Hamburg" annotation(HideResult=true);
    parameter SI.Volume Volume_tank= 2 "|Storage|Volume of the storage tank" annotation(HideResult=true);
    parameter Real p_Volume[3]={0.2,0.3,0.5} "|Storage|Proportion of the total volume for the three parts of the tank" annotation(HideResult=true);

    parameter SI.Temperature T_start[storage.N_cv]=fill(273.15 + 60, storage.N_cv) "|Storage|Temperatures at initalization" annotation(HideResult=true);

    parameter SI.Length h_tank=1 "|Storage|height of tank" annotation(HideResult=true);
    parameter SI.CoefficientOfHeatTransfer U_wall=0.5 "|Storage|Coefficient of heat transfer from wall to ambient " annotation(HideResult=true);
    parameter SI.ThermalConductivity k=0.6 "|Storage|Thermal conductivity of fluid in storage" annotation(HideResult=true);
    parameter SI.Density rho=1e3 "|Storage|Density of fluid in storage" annotation(HideResult=true);
    parameter SI.SpecificHeatCapacity c_v=4.185e3 "|Storage|Heat capacity of fluid in storage" annotation(HideResult=true);
    replaceable model CostStatisticsModel =
      TransiEnt.Components.Statistics.ConfigurationData.StorageCostSpecs.SmallHotWaterStorage  "|Statistics| Model for global cost calculation of the storage"  annotation (choicesAllMatching=true, Dialog(group="Statistics"),HideResult=true);
    replaceable model CostRecordSolarThermal =
      TransiEnt.Components.Statistics.ConfigurationData.GeneralCostSpecs.SolarThermal  "|Statistics|Solar thermal cost specification" annotation (__Dymola_choicesAllMatching=true,HideResult=true);


  // _____________________________________________
  //
  //           Variables
  // _____________________________________________

     SI.Energy E_heating;
     SI.Energy E_hotwater;
     SI.Energy E_solar;
     SI.Energy E_boiler;

     Real SolarFraction=E_solar/(E_hotwater+E_heating+0.001);

     SI.MassFlowRate m_flow_consumer;
     SI.MassFlowRate m_flow_hotwater;
     SI.Power Q_flow_boiler;

     SI.HeatFlowRate HeatingSolar;
     SI.MassFlowRate m_flow_fuel "Fuel mass flow rate";

     Real solarCoverage;

    SI.SpecificEnthalpy CalorificValue=if referenceNCV then TransiEnt.Basics.Functions.GasProperties.getRealGasNCV_xi(
      FuelMedium,
      xi_in=xi_fuel[1:FuelMedium.nc - 1],
      NCVIn=0) else TransiEnt.Basics.Functions.GasProperties.getRealGasGCV_xi(
      FuelMedium,
      xi_in=xi_fuel[1:FuelMedium.nc - 1],
      GCVIn=0) "Calorific value of fuel" annotation(HideResult=true);

    SI.MassFraction xi_fuel[FuelMedium.nc] "[CH4, C2H6, C3H8, C4H10, N2, CO2, H2] Fuel gas mass fractions" annotation(HideResult=true);

  // _____________________________________________
  //
  //           Instances of other Classes
  // _____________________________________________

  Modelica.Blocks.Sources.RealExpression heatFlowRate_boiler(y=Q_flow_boiler)  annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-48,-50})));

  IntegraNet.Storage.Heat.HeatStorageStratified_constProp.StratifiedHotWaterStorage_L3_noFluidPorts storage(
    N_cv=3,
    Volume_segments=p_Volume*Volume_tank,
    V=Volume_tank,
    h=h_tank,
    T_start=T_start,
    U_wall=U_wall,
    k=k,
    rho=rho,
    c_v=c_v,
    redeclare model CostStatisticsModel = CostStatisticsModel) annotation (Placement(transformation(extent={{-82,-14},{-60,8}})));

  IntegraNet.Producer.Heat.SolarThermal.solarCollector_energybased solarCollector(
    G_min=controller.G_min,
    Q_flow_n=Q_flow_n,
    area=area,
    eta_0=eta_0,
    a1=a1,
    a2=a2,
    c_eff=c_eff,
    longitude_local=longitude_local,
    longitude_standard=longitude_standard,
    totaldays=totaldays,
    latitude=latitude,
    slope=slope,
    surfaceAzimuthAngle=surfaceAzimuthAngle,
    redeclare model Skymodel = Skymodel,
    reflectance_ground=reflectance_ground,
    direct_normal=direct_normal,
    redeclare model CostRecordSolarThermal = CostRecordSolarThermal) annotation (Placement(transformation(extent={{62,-24},{80,-6}})));
  Modelica.Blocks.Sources.RealExpression T_in1(y=storage.port[3].T)
                                                           annotation (Placement(transformation(extent={{-6.5,-4.5},{6.5,4.5}},
        rotation=0,
        origin={54.5,16.5})));

  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow2 annotation (Placement(transformation(extent={{-30,28},{-44,42}})));
  Modelica.Blocks.Sources.RealExpression boiler(y=min(Q_flow_n_boiler, max(0, 0.5*4190*(T_boiler - storage.port[1].T))))
                                                                                                     annotation (Placement(transformation(extent={{30,40},{14,52}})));
  Modelica.Blocks.Sources.RealExpression Q_heating1(y=-HeatingSolar - Q_heating2.y)        annotation (Placement(transformation(extent={{30,30},{14,42}})));
  Modelica.Blocks.Math.Add3 add
                               annotation (Placement(transformation(extent={{-12,30},{-22,40}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow annotation (Placement(transformation(extent={{-30,-2},{-44,12}})));
  Modelica.Blocks.Sources.RealExpression Q_solar1(y=controller.P_drive*4185*(solarCollector.T_out - storage.port[2].T)) annotation (Placement(transformation(extent={{30,8},{14,20}})));
  Modelica.Blocks.Math.Add3 add1
                                annotation (Placement(transformation(extent={{-12,0},{-22,10}})));
  Modelica.Blocks.Sources.RealExpression Q_heating2(y=max(-HeatingSolar, min(0, -m_flow_consumer*4185*(storage.port[2].T - T_return))))           annotation (Placement(transformation(extent={{30,-2},{14,10}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow prescribedHeatFlow1 annotation (Placement(transformation(extent={{-28,-30},{-42,-16}})));
  Modelica.Blocks.Sources.RealExpression Q_solar2(y=controller.P_drive*4185*(storage.port[2].T - storage.port[3].T)) annotation (Placement(transformation(extent={{30,-24},{14,-12}})));
  Modelica.Blocks.Math.Add add2 annotation (Placement(transformation(extent={{-12,-28},{-22,-18}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW3(y=max(-demand[3], min(0, -m_flow_hotwater*4185*(storage.port[3].T - 12 - 273.15)))) annotation (Placement(transformation(extent={{30,-34},{14,-22}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW2(y=max(-demand[3] - Q_DHW3.y, min(0, -m_flow_hotwater*4185*(storage.port[2].T - storage.port[3].T)))) annotation (Placement(transformation(extent={{30,-12},{14,0}})));
  Modelica.Blocks.Sources.RealExpression Q_DHW1(y=-(demand[3] + Q_DHW2.y + Q_DHW3.y)) annotation (Placement(transformation(extent={{30,20},{14,32}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature(T=T_room) annotation (Placement(transformation(extent={{4,-4},{-4,4}},
        rotation=90,
        origin={-70,22})));

  Producer.Heat.Gas2Heat.SmallGasBoiler.GasBoiler_energybased gasBoiler(CalorificValue=CalorificValue) annotation (Placement(transformation(extent={{-18,-70},{2,-50}})));

  TransiEnt.Components.Boundaries.Electrical.ApparentPower.ApparentPower apparentPower(useInputConnectorQ=false) annotation (Placement(transformation(extent={{-92,-60},{-72,-40}})));

  TransiEnt.Producer.Heat.SolarThermal.Control.ControllerPumpSolarCollectorTandG
                                                  controller annotation (Placement(transformation(extent={{74,34},{100,54}})));
  Modelica.Blocks.Math.Gain P_drive2m_flow(k=-0.0205)
    annotation (Placement(transformation(extent={{66,52},{58,60}})));
equation

  // _____________________________________________
  //
  //            Characteristic equations
  // _____________________________________________

  Q_solar1.y+Q_solar2.y=der(E_solar);
  Q_flow_boiler=der(E_boiler);
  demand[2]=der(E_heating);
  -Q_DHW1.y-Q_DHW2.y-Q_DHW3.y=der(E_hotwater);

  solarCoverage=if SpaceHeating then E_solar/(E_heating+E_hotwater+0.001) else E_solar/(E_hotwater+0.001);

   m_flow_hotwater=demand[3]/(4185*(storage.port[1].T - (12 + 273.15)));
   m_flow_fuel= if SpaceHeating then boiler.y/gasBoiler.CalorificValue*1/eta else (boiler.y+demand[2])/gasBoiler.CalorificValue*1/eta;

   xi_fuel[1:FuelMedium.nc - 1] = inStream(gasPortIn.xi_outflow);
   xi_fuel[end] = 1-sum(xi_fuel[1:FuelMedium.nc - 1]);

 if SpaceHeating==true then
   HeatingSolar=demand[2];
   m_flow_consumer=demand[2]/(4185*(storage.port[1].T - T_return));
   Q_flow_boiler=boiler.y;
 else
   HeatingSolar=0;
   m_flow_consumer=0;
   Q_flow_boiler=boiler.y+demand[2];
 end if;

 // _____________________________________________
 //
 //            Connect statements
 // _____________________________________________


  connect(storage.heatPortAmbient, fixedTemperature.port) annotation (Line(points={{-71,6.35},{-70,6.35},{-70,18}},              color={191,0,0}));
  connect(prescribedHeatFlow2.Q_flow, add.y) annotation (Line(points={{-30,35},{-30,35},{-22.5,35}},            color={0,0,127}));
  connect(prescribedHeatFlow.Q_flow, add1.y) annotation (Line(points={{-30,5},{-22.5,5}},                         color={0,0,127}));
  connect(prescribedHeatFlow1.Q_flow, add2.y) annotation (Line(points={{-28,-23},{-22.5,-23}},                     color={0,0,127}));
  connect(boiler.y, add.u1) annotation (Line(points={{13.2,46},{6,46},{6,39},{-11,39}}, color={0,0,127}));
  connect(Q_heating1.y, add.u2) annotation (Line(points={{13.2,36},{6,36},{6,35},{-11,35}},
                                                                                        color={0,0,127}));
  connect(Q_DHW1.y, add.u3) annotation (Line(points={{13.2,26},{6,26},{6,31},{-11,31}},       color={0,0,127}));
  connect(Q_solar1.y, add1.u1) annotation (Line(points={{13.2,14},{6,14},{6,9},{-11,9}},  color={0,0,127}));
  connect(Q_heating2.y, add1.u2) annotation (Line(points={{13.2,4},{-11,4},{-11,5}},             color={0,0,127}));
  connect(Q_DHW2.y, add1.u3) annotation (Line(points={{13.2,-6},{5.6,-6},{5.6,1},{-11,1}},            color={0,0,127}));
  connect(Q_solar2.y, add2.u1) annotation (Line(points={{13.2,-18},{2,-18},{2,-20},{-11,-20}},  color={0,0,127}));
  connect(Q_DHW3.y, add2.u2) annotation (Line(points={{13.2,-28},{14,-28},{4,-28},{2,-28},{2,-26},{-11,-26}},
                                                                                                  color={0,0,127}));
  connect(solarCollector.T_in, T_in1.y) annotation (Line(points={{64.16,-7.08},{64.16,6},{64,6},{64,16.5},{61.65,16.5}},
                                                                                                                     color={0,0,127}));
  connect(heatFlowRate_boiler.y, gasBoiler.heatFlowRate) annotation (Line(points={{-37,-50},{-8,-50},{-8,-50.6}}, color={0,0,127}));
  connect(gasBoiler.gasPortIn, gasPortIn) annotation (Line(
      points={{0,-69.6},{38,-69.6},{38,-96},{80,-96}},
      color={255,255,0},
      thickness=1.5));
  connect(apparentPower.epp, epp) annotation (Line(
      points={{-92,-50},{-92,-74.05},{-80,-74.05},{-80,-98}},
      color={0,127,0},
      thickness=0.5));
  connect(demand[1], apparentPower.P_el_set) annotation (Line(points={{0,108},{-28,108},{-28,52},{-48,52},{-88,52},{-88,-38}}, color={0,127,127}));

  connect(prescribedHeatFlow1.port, storage.port[3]) annotation (Line(points={{-42,-23},{-56,-23},{-56,4.66333},{-60.88,4.66333}},color={191,0,0}));
  connect(prescribedHeatFlow.port, storage.port[2]) annotation (Line(points={{-44,5},{-50,5},{-50,3.93},{-60.88,3.93}},color={191,0,0}));
  connect(prescribedHeatFlow2.port, storage.port[1]) annotation (Line(points={{-44,35},{-54,35},{-54,3.19667},{-60.88,3.19667}},color={191,0,0}));
  connect(T_in1.y, controller.T_in) annotation (Line(points={{61.65,16.5},{64,16.5},{64,46},{75.7333,46}},
                                                                                                      color={0,0,127}));
  connect(T_in1.y, controller.T_stor) annotation (Line(points={{61.65,16.5},{64,16.5},{64,34},{75.7333,34}},
                                                                                                        color={0,0,127}));
  connect(controller.G_total, solarCollector.G) annotation (Line(points={{75.7333,38.1667},{70,38.1667},{70,0},{78.92,0},{78.92,-7.26}}, color={0,0,127}));
  connect(solarCollector.T_out, controller.T_out) annotation (Line(points={{76.58,-7.26},{76.58,-8},{78,-8},{76,-8},{76,-2},{68,-2},{68,42.3333},{75.7333,42.3333}}, color={0,0,127}));
  connect(controller.P_drive, P_drive2m_flow.u) annotation (Line(points={{75.7333,49},{76,49},{76,56},{66.8,56}}, color={0,0,127}));
  connect(P_drive2m_flow.y, solarCollector.m_flow) annotation (Line(points={{57.6,56},{46,56},{46,0},{62.18,0},{62.18,-7.26}}, color={0,0,127}));
  annotation (Icon(graphics={      Ellipse(
          lineColor={0,125,125},
          fillColor={255,255,255},
          fillPattern=FillPattern.Solid,
          extent={{-100,100},{100,-100}}),
        Bitmap(
          extent={{-6,30},{84,-88}},
          imageSource=""),
        Polygon(
          points={{-92,4},{-18,-42},{24,50},{-34,84},{-92,4}},
          lineColor={0,0,0},
          smooth=Smooth.None,
          fillPattern=FillPattern.Solid,
          fillColor={95,95,95},
          lineThickness=0.5),
        Line(
          points={{-96,14},{-22,-34},{-16,-20},{-76,14},{-68,28},{-8,-6},{0,8},{-60,40},{-52,52},{6,20},{12,34},{-44,64},{-36,76},{52,24}},
          color={255,255,255},
          smooth=Smooth.None),
        Line(
          points={{24,44},{28,38},{22,38}},
          color={0,0,0},
          smooth=Smooth.None),
        Line(
          points={{-94,18},{-92,12},{-98,12}},
          color={0,0,0},
          smooth=Smooth.None),
        Line(points={{18,44},{28,38}}, color={0,0,0}),
        Line(points={{-98,16},{-86,8}}, color={0,0,0})}), Documentation(info="<html>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">1. Purpose of model</span></b></p>
<p>Combination of solarthermal panel, thermal storage tank and boiler models to be used in the energyConverter.</p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">2. Level of detail, physical effects considered, and physical insight</span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">(Purely technical component without physical modeling.)</span></p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">3. Limits of validity </span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">(Purely technical component without physical modeling.)</span></p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">4. Interfaces</span></b></p>
<p>IntegraNet.Basics.Interfaces.General.PowerIn <b>demand</b></p>
<p>TransiEnt.Basics.Interfaces.Electrical.ApparentPowerPort <b>epp - connection to electrical grid</b></p>
<p>TransiEnt.Basics.Interfaces.Gas.RealGasPortIn <b>gasPortIn - connection to gas grid</b></p>
<p><br><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">5. Nomenclature</span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">(no remarks)</span></p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">6. Governing Equations</span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">(no remarks)</span></p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">7. Remarks for Usage</span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">The model contains models for a solarthermal panel, a stratified storage tank, a gas boiler and a controller for the operation of the solar thermal panel. Different control modes can be selected. </span></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">No fluid flow into the storage tank is considered, therefore the energy input into each layer into the storage tank is defined by energy balances.</span></p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">8. Validation</span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">(no remarks)</span></p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">9. References</span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">(no remarks)</span></p>
<p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">10. Version History</span></b></p>
<p><span style=\"font-family: MS Shell Dlg 2;\">Model created by Anne Hagemeier, Fraunhofer UMSICHT in 2017</span></p>
</html>"));
end solarThermal_Boiler;
