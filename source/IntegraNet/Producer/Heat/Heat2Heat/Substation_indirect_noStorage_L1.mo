...
model Substation_indirect_noStorage_L1 "Simple model of a Substation with indirect connection. Ideal Temperature difference assumed - Based upon the calculation of needed mass flow for a given temperature difference at Heat demand"
 ...
 // _____________________________________________
  ...
  ClaRa.Components.BoundaryConditions.BoundaryVLE_Txim_flow m_flow_out(
    ...
    p_nom=simCenter.p_nom[1]) annotation (Placement(transformation(
        ...
        rotation=-90,
        origin={100,-70})));
  IntegraNet.Producer.Heat.Heat2Heat.Indirect_HEX_L1 room_heating_hex(
    T_start=T_start,
    dT_soll=dT,
  ...
  Modelica.Blocks.Math.Gain signChanger_1(k=-1) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-27.5,-4.5})));
  IntegraNet.Producer.Heat.Heat2Heat.Indirect_HEX_L1 domestic_hot_water_hex(
    T_start=T_start,
    dT_soll=dT,
  ...
  ClaRa.Components.Sensors.SensorVLE_L1_T T_out_sub(unitOption=1)
      ...
      Placement(transformation(
        ...
        rotation=0,
        origin={66,-60})));
  Statistics.LocalCollector output_DHW_DHN(typeOfResource=IntegraNet.Statistics.TypeOfResource.q_output_DHN) annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
  TransiEnt.Components.Statistics.Collectors.LocalCollectors.CollectHeatingPower collectHeatingPower(typeOfResource=TransiEnt.Basics.Types.TypeOfResource.Generic) annotation (Placement(transformation(extent={{-140,-100},{-120,-80}})));
...
equation
 ...
 collectHeatingPower.heatFlowCollector.Q_flow=room_heating_hex.Q_withdrawal + domestic_hot_water_hex.Q_withdrawal;

 output_DHW_DHN.flowCollector.unit_flow = room_heating_hex.Q_withdrawal + domestic_hot_water_hex.Q_withdrawal;
 connect(output_DHW_DHN.flowCollector,statistics_collector.flowCollector[IntegraNet.Statistics.TypeOfResource.q_output_DHN]);
  ...
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-140,-100},{140,100}}),
        ...
        Text(
          extent={{-30,86},{36,92}},
          lineColor={0,0,0},
          textString="calculates needed mass flow
for given dT
")}),
...
<p>(no remarks)</p><p><b><span style=\"font-family: MS Shell Dlg 2; color: #008000;\">7. Remarks for Usage</span></b> </p>
