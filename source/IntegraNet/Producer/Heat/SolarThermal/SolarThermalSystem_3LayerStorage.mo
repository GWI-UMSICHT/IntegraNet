...
model SolarThermalSystem_3LayerStorage
...
replaceable model Skymodel =
    ...
    parameter Real p_Volume[storage.N_cv]={0.2,0.3,0.5} "|Storage|Proportion of the total volume for the three parts of the tank";
...
//                                                                                        "|Statistics|Solar thermal cost specification" annotation (__Dymola_choicesAllMatching=true);
  ...
  Producer.Heat.Gas2Heat.SmallGasBoiler.GasBoiler_energybased gasBoiler(FuelMedium=FuelMedium, eta=eta) if fuel==FT.Gas annotation (Placement(transformation(extent={{-18,-82},{2,-62}})));
  GenericFuel2Heat.GenericBoiler oilBoiler(FuelType=FT.FuelType.Oil, eta=eta) if fuel==FT.Oil annotation (Placement(transformation(extent={{8,-90},{28,-70}})));
  GenericFuel2Heat.GenericBoiler pelletBoiler(FuelType=FT.Pellets, eta=eta) if fuel==FT.Pellets annotation (Placement(transformation(extent={{38,-90},{58,-70}})));
...
equation
 ...
 heating_collector.flowCollector.unit_flow = Q_solar1.y+Q_solar2.y;
 connect(heating_collector.flowCollector,statistics_collector.flowCollector[IntegraNet.Statistics.TypeOfResource.q_output_SolarThermal]);
  ...
  connect(solarCollector.T_in, T_in1.y) annotation (Line(points={{64.16,-7.08},{64.16,6},{64,6},{64,16.5},{61.65,16.5}},
                                                                                                                     color={0,0,127}));
  connect(heatFlowRate_boiler.y, gasBoiler.heatFlowRate) annotation (Line(points={{-37,-50},{-8,-50},{-8,-62.6}}, color={0,0,127}));
  connect(gasBoiler.gasPortIn, gasPortIn) annotation (Line(
      points={{0,-81.6},{0,-98},{76,-98}},
      color={255,255,0},
      thickness=1.5));
  ...
  connect(heatFlowRate_boiler.y, oilBoiler.Q_Demand) annotation (Line(points={{-37,-50},{18,-50},{18,-70}}, color={0,0,127}));
  connect(heatFlowRate_boiler.y, pelletBoiler.Q_Demand) annotation (Line(points={{-37,-50},{48,-50},{48,-70}}, color={0,0,127}));
